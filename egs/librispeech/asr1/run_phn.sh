...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in test_clean train_clean_100 dev_clean; do
        make_fbank.sh --cmd "$train_cmd" --nj 8 --write_utt2num_frames true --write_utt2num_frames true --fs 16000 --fmin 20 --n-mels 80 --n-fft 512 --n-shift 160 --win-length 400 data/${x} exp/make_fbank/${x} ${fbankdir}
    done
