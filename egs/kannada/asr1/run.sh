...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    local/data_prep.sh downloads data
fi
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in train_all dev_all; do
        local/make_torchaudio_fbank.sh --cmd "$train_cmd" --nj 16 --write_utt2num_frames true \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    text2token.py -s 1 -n 1 data/${train_set}/text --trans_type "phn" | cut -f 2- -d" " | tr " " "\n" \
